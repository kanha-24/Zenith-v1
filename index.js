...
if (missingEnv.length > 0) {
  ...
  printInfo('Create a .env file from .env.example before starting Zenith.');
  process.exit(1);
}
...
process.emitWarning = (warning, ...args) => {
  ...
  if (msg.includes('value provided is not in a recognized RFC2822 or ISO format')) return;
...
client.once('clientReady', async () => {
  ...
  client.user.setPresence({
    status: 'idle',
    activities: [{
      name: `${config.PREFIX}help | @Zenith`,

    ...
    async execute(message, args) {
        ...
        if (!args.length) {
            const container = new ContainerBuilder()
                ...
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('-# Admin restricted access | Zenith')
                );
        ...
        if (totalPages === 1) {
            const container = new ContainerBuilder()
                ...
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(`-# ${statusText} · ${executionTime.toFixed(0)}ms · Type: \`${resultType}\` | Zenith`)
                );
        ...
        const renderPage = (pageIndex, pageData, state) => {
            const container = new ContainerBuilder()
                ...
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(`-# Page ${pageData.pageNum}/${totalPages} · ${statusText} · ${executionTime.toFixed(0)}ms · Type: \`${resultType}\` | Zenith`)
                );
